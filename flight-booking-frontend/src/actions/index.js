...
export const storeFlight = (flightId) => {
  ...
  return async (dispatch) => {
    try {
      const res = await axios.get("http://localhost:9000/tickets/" + flightId);
...
export const searchShow = (data) => {
  return async (dispatch) => {
    try {
      const res = await axios.post(
        "http://localhost:9000/tickets/search",
        data
      );
      console.log(res.data);
      if (res.data.length > 0) {
        ...
        dispatch({
          type: FLIGHT_ERROR,
          payload: "Could not get any shows",
        });
      }
      // return res.data;
    } catch (error) {
